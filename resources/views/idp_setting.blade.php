...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-danger">IDP Data Master</h1>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <form action="{{ route('idp.setting.store') }}" method="POST" class="form-confirm">
                            ...
                            <div class="mb-4"><label for="percentage_slider" class="form-label">Percentage: <span id="slider_value" class="fw-bold text-danger">50%</span></label><input type="range" name="percentage" class="form-range" min="10" max="100" step="10" id="percentage_slider" value="50"></div>
                            <div class="text-end"><button type="submit" class="btn btn-danger">Add Model</button></div>
                        </form>
                    </div>
                </div>
            </div>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <form action="{{ route('idp.setting.destroy', $model->id) }}" method="POST" class="d-inline form-delete">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-outline-danger">Delete</button></form>
                                        </td>
                                    </tr>
                                    @empty
                ...
                <form action="{{ route('idp.setting.index') }}" method="GET">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="input-group" style="max-width: 300px;">
                            ...
                            <button class="btn btn-outline-secondary btn-sm" type="submit"><i class="bi bi-search"></i></button></div>
                    </div>
                </form>
                <div class="table-responsive">
                    <table class="table table-hover align-middle"><thead class="table-custom-header"><tr><th>No</th><th>Value</th><th class="text-center" style="width: 120px;">Action</th></tr></thead>
                    <tbody>@forelse($competencyNames as $item)<tr><td>{{ $loop->iteration + ($competencyNames->currentPage() - 1) * $competencyNames->perPage() }}</td><td>{{ $item->value }}</td><td class="text-center"><button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#editMasterModal" data-id="{{ $item->id }}" data-type="{{ $item->type }}" data-value="{{ $item->value }}">Edit</button><form action="{{ route('idp.setting.master.destroy', $item->id) }}" method="POST" class="d-inline form-delete-master">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-outline-danger">Delete</button></form></td></tr>@empty<tr><td colspan="3" class="text-center p-3">No data.</td></tr>@endforelse</tbody></table>
                </div>
...
<div class="tab-pane fade @if($activeTab === 'pills-programs') show active @endif" id="pills-programs" role="tabpanel">
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            ...
            <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#addMasterModal" data-type="development_program">+ Add New</button>
        </div>
        <div class="card-body">
            {{-- Form for filters --}}
            <form action="{{ route('idp.setting.index') }}" method="GET">
            ...
            {{-- Pagination --}}
            <div class="mt-3">
                {!! $developmentPrograms->withQueryString()->links('vendor.pagination.custom') !!}
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Script untuk Modal "Edit Master Data" ---
        const editMasterModal = document.getElementById('editMasterModal');
        if (editMasterModal) {
            // Gunakan variabel yang benar dari controller
